...
export class RealtimeFeedClient {
  constructor({ url, symbol, onTick, onStatus, heartbeatMs = DEFAULT_HEARTBEAT_MS, staleMs = DEFAULT_STALE_MS, reconnectMs = 2000 }) {
    ...
    this.url=url; this.symbol=symbol; this.onTick=onTick; this.onStatus=onStatus; this.heartbeatMs=heartbeatMs; this.staleMs=staleMs; this.reconnectMs=reconnectMs; this.socket=null; this.heartbeat=null; this.staleTimer=null; this.closed=false; this.retry=0; this.lastTick=0;
  }
  connect() { this.closed=false; this.clearTimers(); this.emit('CONNECTING'); this.socket=new WebSocket(this.url); this.socket.onopen=()=>{this.retry=0; this.emit('CONNECTED'); this.subscribe(); this.heartbeat=setInterval(()=>this.ping(),this.heartbeatMs); this.staleTimer=setInterval(()=>{if(this.lastTick && Date.now()-this.lastTick>this.staleMs)this.emit('STALE');},Math.max(5000,this.staleMs/3));}; this.socket.onmessage=e=>{const m=this.parse(e.data); if(!m)return; const t=normalizeTick(m,this.symbol); if(!t)return; this.lastTick=t.timestamp; this.onTick?.(t); this.emit('LIVE');}; this.socket.onerror=()=>this.emit('ERROR'); this.socket.onclose=()=>{this.clearTimers(); if(this.closed)return; this.emit('DISCONNECTED'); const delay=Math.min(30000,this.reconnectMs*2**this.retry++); setTimeout(()=>!this.closed&&this.connect(),delay);}; return this; }
