...
export class MarketDataProvider {
  ...
  async candles(symbol, interval, outputsize = 200, { signal } = {}) {
    ...
    const candles = payload.candles.map(c => ({time:c.datetime,datetime:c.datetime,open:Number(c.open),high:Number(c.high),low:Number(c.low),close:Number(c.close),volume:Number(c.volume||0)})).filter(c=>[c.open,c.high,c.low,c.close].every(Number.isFinite));
