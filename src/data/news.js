const CACHE_MS=60000;const cache=new Map();
...
export async function loadNewsRisk(symbol){
  ...
  // GitHub Pages cannot execute the Netlify function path. Keep the dashboard
  // usable and explicitly return UNKNOWN instead of generating a misleading risk state.
  ...
  const currencies=currenciesForSymbol(symbol),params=new URLSearchParams({symbol:key,currencies:currencies.join(','),horizonMinutes:'180'}),base=globalThis.MARKET_API_BASE_URL||'/.netlify/functions',response=await fetch(`${base.replace(/\/$/,'')}/calendar?${params}`);
