...
export function sessionContext(date = new Date()) {
  ...
  return { active: sessions.length ? sessions.join(' + ') : 'OFF-SESSION', overlap: sessions.length > 1, utcHour };
}
...
export function setupQuality({ analysis, date = new Date(), newsRisk = 'UNKNOWN' }) {
  const session = sessionRisk(date), execution = analysis?.execution, bias = analysis?.higherTimeframeBias;
  let score = Number(execution?.score || 0); const reasons = [];
  if (bias === 'CONFLICT' || bias === 'RANGE') { score -= 20; reasons.push('Higher-timeframe structure is not directional.'); }
  if (session.level === 'ELEVATED') { score -= 10; reasons.push(session.reason); }
  if (newsRisk === 'HIGH') { score -= 30; reasons.push('High-impact news risk is active or imminent.'); }
  else if (newsRisk === 'MEDIUM') { score -= 12; reasons.push('Moderate event risk; execution should be more selective.'); }
  if (execution?.decision === 'NO TRADE') reasons.push(execution.reason);
  const finalScore = clamp(score), grade = finalScore >= 85 ? 'A' : finalScore >= 75 ? 'B' : finalScore >= 65 ? 'C' : 'D';
  const blocked = execution?.decision === 'NO TRADE' || finalScore < 65 || bias === 'CONFLICT' || newsRisk === 'HIGH';
  return { score: finalScore, grade, blocked, verdict: blocked ? 'NO TRADE' : analysis.decision, session, reasons };
}
